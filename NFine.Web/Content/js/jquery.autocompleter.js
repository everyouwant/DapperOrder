...
;(function ($, window) {
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < limit) {
                        switch (i) {
                            case 0:
                                if (source[item].label.toUpperCase().search(query) === 0 || (source[item].cname != undefined && source[item].cname.toUpperCase().search(query) === 0) || (source[item].code != undefined && source[item].code.toUpperCase().search(query) === 0)) {
                            ...
                            case 1:
                                if (source[item].label.toUpperCase().search(query) !== -1 || (source[item].cname != undefined && source[item].cname.toUpperCase().search(query) === 0) || (source[item].code != undefined && source[item].code.toUpperCase().search(query) === 0)) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
                            break;
    ...
    function _setValue(data) {
        if (data.$selected) {
            ...
            var text = data.$selected.attr('data-label');
            data.$node.val(text);
        } else {
            if (data.hintText && !data.$autocompleter.find('.autocompleter-hint').hasClass('autocompleter-hint-show')) {
